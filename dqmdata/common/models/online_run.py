...
def add_run(run, overwrite=False, test=False ):
	...
	db_access = False # Change to True when you figure out how to query the DB!
	if db_access:
		runinfo = this_run.query_runinfo(run, keys=["CMS.LVL0:START_TIME_T", "CMS.LVL0:TRIGGER_MODE_AT_START", "CMS.DAQ:NB_EVENTS_BUILT"])
		# Time example: 2018-06-21 15:40:22 CEST
		this_run.start_time        = datetime.strptime(runinfo["CMS.LVL0:START_TIME_T"], '%Y-%m-%d %H:%M:%S %Z')
		this_run.mode     = runinfo["CMS.LVL0:TRIGGER_MODE_AT_START"]
		this_run.events   = int(runinfo["CMS.DAQ:NB_EVENTS_BUILT"])
		
		# Luminosity
		this_run.ytd_lumi = this_run.get_ytd_lumi(run, overwrite_lumi_cache=False)
	else:
		# Fill in dummy data
		this_run.start_time        = datetime.strptime("2000-01-01 12:00:00 CEST", '%Y-%m-%d %H:%M:%S %Z')
		this_run.mode     = "collisions2018"
		this_run.events   = 0
		this_run.ytd_lumi = 0.
...
class OnlineRun(db.Model):
	...
	def query_runinfo(self, run, keys=["CMS.LVL0:START_TIME_T", "CMS.LVL0:TRIGGER_MODE_AT_START", "CMS.DAQ:NB_EVENTS_BUILT"], overwrite_runinfo_cache=True):
