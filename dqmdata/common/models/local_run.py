...
def add_run(run, overwrite=False, test=False ):
	...
	if q.count() > 0:
		if not overwrite:
			print "[LocalRun::add_run] WARNING : Run {} already exists in db. Use overwrite=True to overwrite.".format(run)
			return
		else:
	...
	if db_access:
		...
		# Fill in dummy data
		start_time                      = datetime.strptime("2000-01-01 12:00:00 CEST", '%Y-%m-%d %H:%M:%S %Z')
		this_run.dqm_task               = "UNKNOWN"
		this_run.local_runkey_selected  = "UNKNOWN"
		this_run.mastersnippet_selected = "UNKNOWN"
		this_run.events                 = 0
		this_run.ytd_lumi               = 0.
