...
def convert_json_to_root(json_data):
	...
	for idx, item in enumerate(json_data['contents']):
		if "obj" in item.keys() and "rootobj" in item.keys():
			bit_array = array('B')
			bit_array.fromstring(bytes.fromhex(item['rootobj']).decode('ascii'))
...
# - project = top level folder in dqmdata, i.e. hcal_local or hcal_online
...
# Download a DQM file as ROOT file... currently broken, fix before using (or use the supported JSON methods above)
# Example URLs
